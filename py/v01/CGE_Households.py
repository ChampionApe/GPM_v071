...
class SimpleRamsey(GmsPython):
	...
	def adjust(self, L2C = None, svngs = None, f = None):
		...
		self.ns.update({k: f"{k}_{self.name}" for k in ('labor','L2C','svngs','s','output_n')})
	...
	def addCalibrationSubsets(self,tree):
		...
		self.s.db[self.n('endo_mu')] = gpyDB_wheels.adj.rc_pd(self.get('map'), self.get('input').rename({'n':'nn'}))
	...
	def initSymbols(self):
		return {self.n('pS'): gpy(pd.Series(1, index = adjMultiIndexDB.mergeDomains([self.get('txE'),self.get('labor')],self.s.db), name=self.n('pS'))),
				...
				self.n('p'): gpy(pd.Series(1, index = adjMultiIndexDB.mergeDomains([self.get('txE'), self.get('output_n')], self.s.db), name = self.n('p')))}
	...
	def groups_(self,m=None):
		return [GmsPy.Group(f"G_{self.name}_exo_always", 
			v =[('pD', self.g('input')),
				...
				('Rrate', None), ('iRate', None), ('crra', self.g('output')), ('frisch', self.g('labor')),('h_tvc', self.g('svngs')), 
