...
class SimpleArmington(GmsPython):
	...
	def groups_(self,m=None):
		return [GmsPy.Group(f"G_{self.name}_exo_always", 
			v =[('p', self.g('nOut')), ('sigma', self.g('sfor_ndom')), ('tauD', self.g('sfor_ndom')), ('tauLump', ('and', [self.g('s'), self.g('tx0E')]))]),
				GmsPy.Group(f"G_{self.name}_endo_always", 
			v =[('qD', ('and', [self.g('sfor_ndom'), self.g('tx0E')])), ('TotalTax', ('and', [self.g('s'), self.g('tx0E')])), ('pD', self.g('sfor_ndom'))]),
